...
labels = ['Fell asleep at(24hr clock) ', 'Got out of bed at ', 'Substance(all lowercase) ', 'Sleep duration (hours) ',
          'Quality of sleep (1 to 10)', 'Stress (1 to 10) ']
...
def submit():
    ...
    if a > 0:
        ...
        label1 = ttk.Label(f2, text='Affecting Factors !', font=('Helvetica', 18, 'bold'))
...
add_day = tk.Button(win, text='Add another Day', command=add_day_func, height=1, width=12)
add_day.grid(row=1, column=0, padx=60, pady=5)

button2 = tk.Button(win, text='Save Data', command=store_data, height=1, width=8)
button2.grid(row=2, column=0, padx=60, pady=5)

button = tk.Button(win, text='Graph', command=submit, height=1, width=8)
